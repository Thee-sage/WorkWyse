...
export function FeatureSection({ icon, title, description, illustration, reversed = false }: FeatureSectionProps) {
  ...
  return (
    <div className={`flex flex-col ${reversed ? "md:flex-row-reverse" : "md:flex-row"} items-center gap-12 md:gap-20`}>
      <ScrollReveal className="flex-1 max-w-lg" direction={reversed ? "right" : "left"} distance={24}>
        <div className="mb-4">{icon}</div>
        <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: "1.75rem", lineHeight: 1.3 }}>
          {firstWords}{" "}
          <MarkerHighlight delay={0.2}>{lastWord}</MarkerHighlight>
        </h2>
        <p className="mt-4" style={{ fontSize: "1rem", lineHeight: 1.8, color: "var(--muted-foreground)" }}>
          {description}
        </p>
        <motion.div
          className="mt-6 h-px"
      ...
      <ScrollReveal className="flex-1 flex justify-center" direction={reversed ? "left" : "right"} distance={24} delay={0.1}>
        {illustration}
      </ScrollReveal>
    </div>
  );
