...
export function ReportCard({
  id, company, companySlug, role, location, date, status, trustScore,
  confirmations, excerpt, index = 0, linkable = true,
}: ReportCardProps) {
  const statusConfig = {
  ...
  const card = (
    ...
    >
      ...
      <div className="absolute top-0 right-0 w-10 h-10 overflow-hidden">
        <div
          className="absolute -top-5 -right-5 w-10 h-10 rotate-45 origin-bottom-left transition-transform duration-200 group-hover:scale-110"
      ...
      <div className="flex items-start justify-between mb-3 gap-3">
        <div className="min-w-0">
          <h3 style={{ fontSize: "1.1rem", fontFamily: "'Playfair Display', serif" }}>
            {companySlug ? (
              <Link
          ...
          <p style={{ fontSize: "0.875rem", color: "var(--muted-foreground)" }}>{role}</p>
        </div>
      ...
      <div className="flex items-center justify-between mt-auto">
        ...
        <span style={{ fontSize: "0.8rem", color: "var(--accent)", transition: "color 0.2s" }} className="hover:opacity-80">
  ...
  if (linkable && id) {
    return <Link href={`/reports/${id}`} style={{ display: "block", height: "100%" }}>{card}</Link>;
  }
